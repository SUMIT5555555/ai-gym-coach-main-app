...
def _get_connection():
    conn = sqlite3.connect(_DB_PATH, check_same_thread=False)
    conn.row_factory = sqlite3.Row  # Fixed: allows dict-like column access
    return conn
...
def create_user(username: str) -> sqlite3.Row:
    ...
    with conn:
        conn.execute(
            "INSERT INTO users (username) VALUES (?)", (username,)
        )  # Fixed: removed duplicate (username) parameter
...
def add_exercise(user_id: int, exercise_name: str, reps: int, sets: int, time: int):
    conn = _get_connection()
