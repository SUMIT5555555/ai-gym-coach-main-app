...
def sync_metrics_update(context):
    ...
    last_saved_metrics = st.session_state.get("last_saved_sets_completed", 0)

    if target_sets > 0 and reps_per_set > 0 and sets_completed > last_saved_metrics:
        newly_completed = sets_completed - last_saved_metrics
        now_ts = time.time()
    ...
    if not pose_detected and st.session_state.get("voice_pipeline"):
        result = st.session_state.voice_pipeline.process_event(
            ...
            exercise=exercise,
            metrics={"issue": "No posed detected! Please step into the camera frame."},
        )
